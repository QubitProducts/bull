...
var Queue = function Queue(name, redisPort, redisHost, redisOptions){
  ...
  getRedisVersion(this.client).then(function(version){
    if(semver.lt(version, MINIMUM_REDIS_VERSION) && !process.env.IGNORE_BULL_REDIS_VERSION){
  ...
  function handleFailed(err){
    ...
    job.takeLock(true /* renwew */, false /* ensureActive */).then( function(lock) {
      return clearTimer()
        ...
        .then(function(){
          return _this.distEmit('failed', job.toJSON(), sanitizeError(error));
        });
    }, function(err){
...
var sanitizeError = function sanitizeError(rawError){
